...
#define BUFFER_BYTESIZE		1920*1080*4	/* Length of the buffers for DMA
					 * transfer
					 */
// byte of a frame
...
static int DoSimplePollTransfer(XAxiCdma *InstancePtr, int Length, int Retries,u8* DestPtr);
static int CheckData(u8 *SrcPtr, u8 *DestPtr, int Length);


int XAxiCdma_SimplePollExample(u16 DeviceId,u8* dstPtr);
...
int XAxiCdma_SimplePollExample(u16 DeviceId,u8* dstPtr)
{
	...
	XAxiCdma_IntrDisable(&AxiCdmaInstance, XAXICDMA_XR_IRQ_ALL_MASK);
		...
		Status = DoSimplePollTransfer(&AxiCdmaInstance,
			BUFFER_BYTESIZE, SubmitTries,dstPtr);
		if (Status != XST_SUCCESS) {
				return XST_FAILURE;
		}
...
//	}
	...
	while (Retries) {
		...
		Status = XAxiCdma_SimpleTransfer(InstancePtr, (UINTPTR)0x0000000080000000,
			(UINTPTR)DestPtr, Length, NULL, NULL);
			print("\r\n one frame transmit done! \r\n");
		if (Status == XST_SUCCESS) {
			break;
		}
